...
class PathFinder:
    ...
    def compute_local_travel_cost(self, city_a, city_b):
        ...
        for start_day in range(self.max_days):
            if city_a != 'wait' and city_b != 'wait':
                ...
                res = self.search(city_a, city_b, start_day)
            else:
                res = None
    ...
    def fall_back_to_greedy(self,initial_cities,costs):
        print("Wait amd save not possible, suggesting greedy route")
        self.build_greedy_route(initial_cities, costs)  # rebuild the travel itinerary  using greedy approach

    def plan_multiplicity(self, route_list, initial_cities, costs):
        """
        Plan the day-wise travel itinerary
        ...
        for city in range(len(route_list) - 1):
            ...
            # compute the local cost needed to travel from start city to end city
            ...
            if city_a == 'wait' or city_b == 'wait':
                ...
                distance_for_day=0
                if travel_days >= 30:
                    self.fall_back_to_greedy(initial_cities, costs)
                    break
                final_path.append("WAIT")  # final path is wait
                # if we have end city by not start city
                if buffer_of_real_cities_stack and (city_b != 'wait'):
                    ...
                    if meta:  # if available, log it
                        for day_index, day_info in meta["daily"].items():
                            ...
                            new_distance =  day_info["distance"]
                            # if the distance exceeds the max distance for the day, move to next day
                            if distance_for_day + new_distance > self.usa_map.max_miles_per_day:
                                travel_days += 1
                                distance_for_day = 0
                                if travel_days >= 30:
                                    self.fall_back_to_greedy(initial_cities, costs)
                                    break
                            distance_for_day += new_distance
                            daily_schedule.append({
                    ...
                    if not final_path:
                        final_path.extend([prev_city, city_b])
                    else:
    ...
    def trip(self, initial_city_list):
        ...
        aggregated = self.plan_multiplicity(travel_route, city_list, costs)  # improved_route
    ...
    def build_cost_aware_route(self, cities, costs):
        ...
        def dp(index, given_day):
            ...
            best = float("inf")  # initialize the best travel cost
